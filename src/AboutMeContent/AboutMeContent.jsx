...
const AboutMeContent = () => {
    ...
    const handlePhotoChange = (e) => {
        ...
        const handleSaveClick = async () => {
            ...
            try {
                const response = await axios.put("https://loggin-api-production.up.railway.app/usuarios", datosActualizados);
            ...
            <div className='about_user_scroll'>
                <div className='aboutMe_user'>
                    <div className="user-container">
                        <img className="user-photo rounded-circle" src={`data:image/jpeg;base64,${user.foto}`} alt="User Photo" />
                        ...
                        <div className='AboutMe_user_info'>
                            ...
                            <div className='aboutMe_user_columnas'>
                                <p>Name:</p>
                                <input className="user-first-name" type="text" value={user.nombre} disabled={!editing} onChange={(e) => setUser({ ...user, nombre: e.target.value })} />
                            </div>
                            <div className='aboutMe_user_columnas'>
                                <p>Last name:</p>
                                <input className="user-last-name" type="text" value={user.apellido} disabled={!editing} onChange={(e) => setUser({ ...user, apellido: e.target.value })} />
                            </div>
                            <div className='aboutMe_user_columnas'>
                                <p>Email:</p>
                                <input className="user-email" type="email" value={user.correo} disabled={!editing} onChange={(e) => setUser({ ...user, correo: e.target.value })} />
                            </div>
                            <div className='aboutMe_user_columnas'>
                                <p>Password:</p>
                                <input className="user-password" type={editing ? "text" : "password"} value={user.contraseña} disabled={!editing} onChange={(e) => setUser({ ...user, contraseña: e.target.value })} />
                            </div>
                            <div className='aboutMe_user_columnas'>
                                <p>Artist name:</p>
                                <input className="user-nickname" type="text" value={user.artistname} disabled={!editing} onChange={(e) => setUser({ ...user, artistname: e.target.value })} />
                            </div>
                        </div>
